...
## 🔜 Lopende onderwerpen
- [x] Diepgaande PPP-scan van `index.html` (02-08-2026): uitgevoerd, bevindingen + prioritering hieronder. Aanleiding: PPP-principe is 01-08-2026 aangescherpt.
  - **Volgorde afgesproken met Ronald:** (1) completeness-meter naar "Mijn profiel" verplaatsen + dode code opruimen, (2) foutafhandeling vriendelijk maken, (3) media-tip-tekst corrigeren.
  - **(1) Presentatie — "Profielstatus volledig/actueel zichtbaar":** dode code gevonden — STEP 7 "Profiel preview" (`step6`) + `renderProfile()` (incl. completeness-meter) wordt nooit aangeroepen sinds "direct naar homepagina na aanmaken" is doorgevoerd. Freshness-bar ("Bijgewerkt op...") zit al wél in "Mijn profiel" (via hergebruik `openMusicianModal()`). Actie: dode code (step6, dot6, `renderProfile()`) verwijderen; completeness-meter (herbouwd op echte Supabase-data i.p.v. registratie-state) toevoegen aan `loadMyProfile()`, alleen zichtbaar voor de eigenaar (niet voor andere kijkers — privacy/relevantie).
  - **(2) Plezier — "foutmeldingen begrijpelijk":** ruwe technische `e.message` werd rechtstreeks getoond in `runSearch()`, `runBandSearch()`, `saveBand()` en `showSaveError()`. Actie: gedeelde `friendlyErrorMessage()`-helper toegevoegd die bekende foutpatronen vertaalt naar begrijpelijke NL-tekst, met nette generieke fallback; toegepast op genoemde plekken. Volledige technische fout blijft gelogd via `console.error` voor debugging.
  - **(3) Prestatie — matchscore-belofte klopt met tekst:** media-tip beweerde "regelmatig updaten geeft voorrang in zoekresultaten", maar technisch bestaat alleen de 6-maanden-inactiviteitsknip. Actie: tekst aangepast zodat die de daadwerkelijke werking beschrijft.
  - **Bewust niet meegenomen in deze ronde (apart gehouden):**
    - Contactfunctie op zoekresultaten (Prestatie: "directe vervolgstap") — staat al gepland, groter dan een scan-fix.
    - Bredere emoji-opschoning (Doel-kaarten/Vibe-tags/media-iconen/succesmomenten) — bestaand backlogpunt, bewust gescheiden gehouden.
    - Label-lengte "Band of artiest"/"Wij zoeken nog" (3 woorden i.p.v. max. 2) — vermoedelijk onvermijdelijk, laag risico, geen actie ondernomen.
    - `alert()` voor validatiefouten (12+ plekken) blijft vooralsnog staan — alleen de fout*inhoud* is vriendelijk gemaakt, de blokkerende alert-vorm zelf is een aparte, grotere UX-wijziging.
- [x] **Restpunten PPP-scan opgelost (02-08-2026):**
  - Dode `updateCompleteness()`-functie + alle 9 aanroepen verwijderd (verwees naar een element dat sinds de dode-code-opruiming nergens meer bestond).
  - Alle 16 `alert()`-aanroepen (validatiefouten in registratie/band aanmaken + de foutafhandeling uit deze scan) vervangen door een nieuwe niet-blokkerende `showToast()`-melding (`#appToast`, CSS `.app-toast`) — donker thema, verdwijnt vanzelf na 3,5 sec. Sluit aan bij Plezier: "licht en leuk", geen onderbrekende technische pop-up.
- [ ] **Volgende grote upgrade (later oppakken, niet nu):** na inloggen direct naar "Mijn profiel" navigeren i.p.v. naar de landingspagina. Raakt navigatiestructuur/onboarding-flow, vereist eigen ontwerpafweging — bewust apart gehouden van de PPP-scan-fixes hierboven.
- [ ] Bredere emoji-opschoning (vervolg op quick-fix 01-08-2026): emoji's bij Doel-kaarten (🎸🎵🎤🌟), Vibe-tags, media-platformherkenning (YouTube/Instagram/SoundCloud/TikTok/Spotify-iconen) en succes-/leegresultaat-illustraties. Bewust niet meegenomen in de quick-fix, want dit is visuele/persoonlijke content i.p.v. besturingselementen — vereist een aparte ontwerpafweging.
